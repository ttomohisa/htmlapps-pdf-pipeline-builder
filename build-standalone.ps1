...
$NodeEditorCorePath = Join-Path $Root "src\vendor\node-editor-core.mjs"
...
if (-not (Test-Path -LiteralPath $NodeEditorCorePath -PathType Leaf)) {
  throw "Node Editor Core source was not found: $NodeEditorCorePath"
}
$nodeEditorCoreSource = [System.IO.File]::ReadAllText($NodeEditorCorePath, [System.Text.Encoding]::UTF8)
$nodeEditorCoreSource = [regex]::Replace($nodeEditorCoreSource, '(?m)\r?\n?export\s+default\s+NodeEditorCore;\s*$', "`n")
if ($nodeEditorCoreSource -match '\bexport\s+default\s+NodeEditorCore\b') {
  throw "ESM export remained in embedded Node Editor Core source."
}
...
$replacements = [ordered]@{
  ...
  "__NODE_EDITOR_CORE_SOURCE__" = $nodeEditorCoreSource
}
